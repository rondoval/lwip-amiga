 ...
 * hooks are implemented in netif_base.c and read the per-interface VID from
 * netif->state (the struct NetIfBase every hardware backend embeds first).
 * See lwipopts.h for the switches.
 */
...
s32_t netifbase_vlan_set(struct netif *netif, struct pbuf *p,
                         const struct eth_addr *src, const struct eth_addr *dst,
                         u16_t eth_type);

/* RX (tagged frames only): return non-zero to accept, 0 to drop. */
int netifbase_vlan_check(struct netif *netif, struct eth_hdr *ethhdr,
                         struct eth_vlan_hdr *vlan);

#define LWIP_HOOK_VLAN_SET(netif, p, src, dst, eth_type) \
    netifbase_vlan_set((netif), (p), (src), (dst), (eth_type))
#define LWIP_HOOK_VLAN_CHECK(netif, eth_hdr, vlan_hdr) \
    netifbase_vlan_check((netif), (eth_hdr), (vlan_hdr))
